...
func (u *UserRepoImpl) UpdateUser(context context.Context, user model.User) (model.User, error) {
	sqlStatement := `
		UPDATE users SET 
			full_name = (CASE WHEN LENGTH(:full_name) < 3 THEN full_name ELSE :full_name END),
			email = (CASE WHEN LENGTH(:email) < 5 THEN email ELSE :email END),
	...
	if err != nil {
		log.Error(err.Error())
		return user, errMess.UserNotUpdated
	}
	if count == 0 {
		return user, errMess.UserNotUpdated
	}
